...
namespace {
    ...
    email::smtp::SMTPServer* g_server = nullptr;
}
...
int main(int argc, char* argv[]) {
    ...
    email::Logger::instance().init(
        email::LogLevel::Info,
        true,
        "/var/log/email_server/smtp.log"
    );

    LOG_INFO("SMTP Server starting...");
    ...
    // Create server
    email::smtp::SMTPServer server(config.smtp(), auth, config.storage().maildir_root);
    g_server = &server;
    ...
    try {
        server.start();
        LOG_INFO("SMTP server started successfully");
    ...
    LOG_INFO("SMTP server shutdown complete");
    return 0;
}
