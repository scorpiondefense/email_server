...
namespace {
    ...
    email::pop3::POP3Server* g_server = nullptr;
}
...
int main(int argc, char* argv[]) {
    ...
    email::Logger::instance().init(
        email::LogLevel::Info,
        true,
        "/var/log/email_server/pop3.log"
    );

    LOG_INFO("POP3 Server starting...");
    ...
    // Create server
    email::pop3::POP3Server server(config.pop3(), auth, config.storage().maildir_root);
    g_server = &server;
    ...
    try {
        server.start();
        LOG_INFO("POP3 server started successfully");
    ...
    LOG_INFO("POP3 server shutdown complete");
    return 0;
}
